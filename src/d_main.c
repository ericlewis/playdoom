...
void D_UpdateWipe(void) {
    boolean done = wipe_ScreenWipe(1);
    I_UpdateNoBlit();
    M_Drawer();
    I_FinishUpdate();
    if (done)
...
void D_DoomLoopRun(void)
{
    ...
    if (entering_cheat || is_wiping) {
        // still want input here
        I_ReadButtons();
    }
...
{
    V_DrawStretched(40, 0, W_CacheLumpName(pagename, PU_CACHE));
    V_BlackBars();
}
...
static void SetMissionForPackName(char *pack_name)
{
    ...
    {
        if (!M_strcasecmp(pack_name, packs[i].name))
        {
...
void D_IdentifyVersion(void)
{
    ...
    if (gamemission == none)
    {
        ...
        {
            if (!M_strncasecmp(lumpinfo[i]->name, "MAP01", 8))
            {
                gamemission = doom2;
                break;
            } 
            else if (!M_strncasecmp(lumpinfo[i]->name, "E1M1", 8))
            {
                gamemission = doom;
                break;
            }
...
static void InitGameVersion(void)
{
    ...
    // "1.7", "1.8", "1.9", "ultimate", "final", and "final2".
    //
...
void D_DoomMain1 (void)
{
    ...
    I_PrintBanner("PlayDoom starting.");
