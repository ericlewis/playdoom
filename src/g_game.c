...
// Due to a lack of run button, we enable run by default, but allow turning it
// off when desired (looking at you, MAP24)
...
void G_BuildTiccmd (ticcmd_t* cmd, int maketic)
{
    ...
    if (!automapactive) {
        if (gamekeydown[KEY_GAME_UP])
            forward += forwardmove[speed];
        if (gamekeydown[KEY_GAME_DOWN])
            forward -= forwardmove[speed];
        if (gamekeydown[KEY_GAME_STRAFELEFT])
            side -= sidemove[speed];
        if (gamekeydown[KEY_GAME_STRAFERIGHT])
            side += sidemove[speed];
        if (gamekeydown[KEY_GAME_FIRE])
            cmd->buttons |= BT_ATTACK;
        if (gamekeydown[KEY_GAME_USE])
            cmd->buttons |= BT_USE;
    }
...
boolean G_Responder (event_t* ev)
{
    ...
    if (!automapactive) {
        if (ev->type == ev_keydown && ev->data1 == KEY_GAME_PREVWEAPON)
            next_weapon = -1;
        else if (ev->type == ev_keydown && ev->data1 == KEY_GAME_NEXTWEAPON)
            next_weapon = 1;
    }
    ...
    switch (ev->type)
    {
      ...
      if (ev->data1 == KEY_GAME_AUTORUN) {
        if (gamestate == GS_LEVEL) {
          autorun = !autorun;

          // Send message to player
          players[consoleplayer].message = autorun ? "RUN SPEED ON" : "RUN SPEED OFF";
        }
      }
...
void G_DoSaveGame (void)
{
    ...
    if (!P_OpenSaveFile(temp_savegame_file, "wb"))
    {
        I_Error("Failed to open '%s' to write savegame.",
                temp_savegame_file);
    }
    ...
    playdate->file->unlink(savegame_file, 0);
    playdate->file->rename(temp_savegame_file, savegame_file);
